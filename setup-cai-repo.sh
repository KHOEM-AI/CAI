...
# ============================================================
# CAI Pro Vision — Repo organizer script
# រត់ក្នុងថត root នៃ repo "CAI" (កន្លែងដែលមាន .git រួចហើយ)
#   cd ~/CAI-new   (ឬថតណាមួយដែល git remote ភ្ជាប់ទៅ KHOEM-AI/CAI)
#   bash setup-cai-repo.sh
# ============================================================
set -e

echo "📁 កំពុងបង្កើតរចនាសម្ព័ន្ធថត..."
mkdir -p backend/routes backend/utils backend/middleware backend/db
mkdir -p frontend
mkdir -p docs
...
cat > backend/utils/tokens.ts << 'CAIEOF'
...
CAIEOF
echo "  ✅ backend/utils/tokens.ts"

cat > backend/utils/audit.ts << 'CAIEOF'
...
CAIEOF
echo "  ✅ backend/utils/audit.ts"

cat > backend/middleware/auth.ts << 'CAIEOF'
...
CAIEOF
echo "  ✅ backend/middleware/auth.ts"

cat > backend/routes/auth.ts << 'CAIEOF'
...
CAIEOF
echo "  ✅ backend/routes/auth.ts"

cat > backend/routes/scans.ts << 'CAIEOF'
import { Router } from 'express';
...
export default router;
CAIEOF
echo "  ✅ backend/routes/scans.ts"

cat > backend/routes/users.ts << 'CAIEOF'
import { Router } from 'express';
import { z } from 'zod';
import { pool } from '../db/pool';
import { authenticate, requireRole } from '../middleware/auth';
...
export default router;
CAIEOF
echo "  ✅ backend/routes/users.ts"

cat > backend/server.ts << 'CAIEOF'
...
CAIEOF
echo "  ✅ backend/server.ts"

# ⚠️ pool.ts មិនត្រូវបានផ្ញើមកទេ — នេះជា implementation ស្តង់ដារ (pg Pool ធម្មតា)
# សូមផ្ទៀងផ្ទាត់ DATABASE_URL ក្នុង .env របស់បងឱ្យត្រឹមត្រូវ
if [ ! -f backend/db/pool.ts ]; then
...
CAIEOF
echo "  ✅ backend/db/pool.ts (ថ្មី — សូមផ្ទៀងផ្ទាត់ DATABASE_URL)"
else
  echo "  ⏭️  backend/db/pool.ts មានរួចហើយ — មិនសរសេរជាន់ទេ"
fi

# ------------------------------------------------------------
# FRONTEND (standalone prototype — ដាច់ដោយឡែកពី App.tsx ដែលមានស្រាប់)
# ------------------------------------------------------------

cat > frontend/CAI_Pro_App.tsx << 'CAIEOF'
...
CAIEOF
echo "  ✅ frontend/CAI_Pro_App.tsx (ដាច់ដោយឡែកពី App.tsx ដើម្បីកុំសរសេរជាន់)"

# ------------------------------------------------------------
# DOCS
# ------------------------------------------------------------

cat > docs/OVERVIEW.md << 'CAIEOF'
# CAI — CAI Pro Vision
...
| ឯកសារ | ខ្លឹមសារ |
|---|---|
| [`docs/ARCHITECTURE.md`](ARCHITECTURE.md) | ផែនទីទាំងមូលនៃប្រព័ន្ធ (Frontend/Backend/Data/Security/AI) |
| [`docs/API.md`](API.md) | Endpoint ទាំងអស់ + Request/Response format |
| [`docs/DATABASE.md`](DATABASE.md) | Table schema ពេញលេញ (users, scans, audit_log ។ល។) |
| [`docs/AUTH_RBAC.md`](AUTH_RBAC.md) | របៀបចូលគណនី, តួនាទី, សិទ្ធិប្រើប្រាស់ |
| [`docs/GPS.md`](GPS.md) | GPS state model + geofencing logic |
| [`docs/AI.md`](AI.md) | AI detection, provenance, roadmap ទៅ custom model |
| [`docs/OFFLINE_SYNC.md`](OFFLINE_SYNC.md) | ដំណើរការ Offline queue + sync |
| [`docs/SECURITY.md`](SECURITY.md) | Threat model — អ្វី Trusted/Untrusted |
...
*ស្វែងរកលក្ខណៈពិសេស, រចនាសម្ព័ន្ធ, Roadmap លម្អិត → សូមមើល [`docs/ARCHITECTURE.md`](ARCHITECTURE.md)*
CAIEOF
echo "  ✅ docs/OVERVIEW.md"

cat > docs/ARCHITECTURE.md << 'CAIEOF'
...
CAIEOF
echo "  ✅ docs/AUTH_RBAC.md"

cat > docs/GPS.md << 'CAIEOF'
# GPS & Geofencing
...
cat > docs/OFFLINE_SYNC.md << 'CAIEOF'
...
```
CAIEOF
echo "  ✅ docs/OFFLINE_SYNC.md"

cat > docs/SECURITY.md << 'CAIEOF'
...
echo ""
echo "✅ រៀបចំរចនាសម្ព័ន្ធរួចរាល់!"
echo ""
echo "📋 ជំហានបន្ទាប់ (ត្រូវពិនិត្យផ្ទាល់ខ្លួន មុន push)៖"
echo "  1. ពិនិត្យ backend/db/pool.ts — ត្រូវប្រាកដថា .env មាន DATABASE_URL ត្រឹមត្រូវ"
echo "  2. App.tsx ដើមនៅ root មិនត្រូវប៉ះពាល់ទេ — CAI_Pro_App.tsx ថ្មីនៅដាច់ក្នុង frontend/"
echo "  3. ពិនិត្យ 'git status' មើលឯកសារថ្មីទាំងអស់មុន commit"
echo ""
read -p "👉 ចង់ធ្វើ git add + commit + push ឥឡូវនេះទេ? (y/n): " confirm
if [ "$confirm" = "y" ] || [ "$confirm" = "Y" ]; then
  git add backend frontend docs
  git commit -m "Organize backend (auth/RBAC), frontend prototype, and full documentation"
  git push
  echo "🎉 Push រួចរាល់!"
else
  echo "⏭️  មិនទាន់ push ទេ — រត់ដោយដៃពេលត្រៀមខ្លួនរួច:"
  echo "     git add backend frontend docs"
  echo "     git commit -m \"Organize backend, frontend, and docs\""
  echo "     git push"
fi
